...
def _neo4j_service_command() -> str:
    ...
    Writes a full ``neo4j.conf`` with memory settings, recovery
    options, and JVM tuning — matching the systemd service template.
    Without explicit memory limits, Neo4j auto-sizes to the compute
    node's full RAM, causing OOM-triggered critical errors.
    """
    from imas_codex.graph.profiles import resolve_neo4j
    ...
    # Full neo4j.conf matching systemd/graph-init defaults.
    # GPFS/NFS environments need fail_on_missing_files=false to
    # survive stale lock/transaction log recovery.
    conf_lines = [
        "# Neo4j configuration (auto-generated by imas-codex serve)",
        "# Memory settings sized for shared HPC compute node",
        "",
        "server.memory.heap.initial_size=1g",
        "server.memory.heap.max_size=4g",
        "server.memory.pagecache.size=4g",
        "",
        "# Bind to all interfaces on exclusive SLURM node",
