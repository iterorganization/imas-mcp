...
def _get_cocos_table(gc: GraphClient) -> list[dict]:
    """Get all COCOS-labeled paths active in the target version."""
    return gc.query("""
        MATCH (p:IMASNode)
...
def _compute_cocos_factors(
    ...
    if from_cocos is None or to_cocos is None or from_cocos == to_cocos:
        return []
    ...
    for p in labeled_paths:
        try:
            ...
            results.append({**p, "factor": "?", "action": "Check manually"})
    return results
...
def _get_version_cocos(gc: GraphClient, version: str) -> int | None:
    ...
    return result[0]["cocos"] if result else None
...
def _render_guide(
    ...
    if cocos_table and from_cocos and to_cocos and from_cocos != to_cocos:
        lines.append("## COCOS Sign-Flip Table\n")
...
def generate_migration_guide(
    ...
    cocos_paths = _get_cocos_table(gc)
