...
**This is an initial triage.** High-value paths will undergo deep enrichment (pattern matching, line counts) and independent re-scoring with that evidence. Your triage determines which paths proceed to that deeper evaluation. Classify accurately and score conservatively — overscoring wastes expensive enrichment on low-value paths.
