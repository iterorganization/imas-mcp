...
### Simulation/Modeling Code (predictive, offline)
- `modeling_code`: Physics simulation/modeling code - runs offline to predict plasma behavior
  - Examples: CHEASE, ASTRA, JOREK, JINTRAC, JETTO, RAPTOR (offline), equilibrium solvers
  - Key signal: Code that takes inputs and predicts/simulates plasma response

### Experimental Code (facility operations, real-time)
- `experimental_code`: Shot processing: intershot analysis, real-time diagnostics, shot review
  - Examples: Real-time MSE processing, intershot equilibrium fits, campaign analysis
  - Key signal: Code that processes actual experimental shots (not simulations)
- `acquisition_code`: Data acquisition and hardware interfaces
  - Examples: DAQ systems, timing code, camera interfaces, hardware control
  - Key signal: Talks to physical hardware, PXI, FPGA, timing systems
- `control_code`: Plasma control systems and real-time feedback
  - Examples: RAPTOR-RT, shape controller, density feedback, MHD suppression
  - Key signal: Real-time control loops, actuator commands, feedback algorithms
- `diagnostic_code`: Diagnostic analysis pipelines (Thomson, bolometry, interferometry, SXR)
  - Key signal: Processes raw diagnostic data into physical quantities
...
- `diagnostic_data`: Raw/calibrated diagnostic measurements (time series, profiles)
  - Key signal: Data from a specific diagnostic system (Thomson, ECE, etc.)
...
### Experimental code indicators:
- `*realtime*`, `*real-time*`, `*rt_*`, `*_rt*` → control_code or experimental_code
- `*controller*`, `*feedback*`, `*actuator*` → control_code
- `*daq*`, `*acquisition*`, `*pxi*`, `*fpga*` → acquisition_code
- `*intershot*`, `*shot_review*`, `*campaign*` → experimental_code
- `*pulse*`, `*shot_*` (in code context) → experimental_code
...
- **Data repositories/containers** (simulation_data, diagnostic_data, experimental_data): Too many files
...
- Purpose is code category (modeling_code, experimental_code, control_code, acquisition_code, diagnostic_code, data_interface, visualization, workflow) AND NO .git folder
...
- **Data containers with many files**: simulation_data, diagnostic_data, experimental_data with total_files > 1000
...
- **Code directories**: Modeling code, experimental code, control code, acquisition code, diagnostic code, data interfaces
