...
@enrich.command("orcid")
@click.option(
    "--limit",
    "-n",
    default=None,
    type=int,
    help="Maximum Person nodes to enrich (default: all without ORCID)",
)
@click.option(
    "--dry-run",
    is_flag=True,
...
def enrich_orcid(limit: int | None, dry_run: bool) -> None:
    ...
    async def _enrich() -> None:
        with GraphClient() as gc:
            # Find Person nodes without ORCID
            query = """
                MATCH (p:Person)
                ...
                RETURN p.id AS person_id,
                       ...
                       p.name AS name,
                       coalesce(p.account_count, 1) AS account_count
                ORDER BY account_count DESC
                LIMIT $limit
            """
            persons = gc.query(query, limit=limit or 10000)

            if not persons:
                console.print("[green]✓[/green] All Person nodes already have ORCID")
                return

            console.print(f"Found {len(persons)} Person nodes without ORCID")

            # Enrich with progress tracking
            enriched = []
            errors = []
            ...
            if enriched:
                table = Table(
                    title="ORCID Enrichment Results", show_header=True, box=None
                )
                table.add_column("Name", style="cyan")
                table.add_column("Email", style="dim")
                table.add_column("ORCID", style="green")

                for entry in enriched[:20]:  # Show first 20
                    table.add_row(
                        entry["name"],
                        entry["email"],
                        entry["orcid"],
                ...
                console.print(
                    f"\n{status} {len(enriched)} Person nodes enriched with ORCID"
                )

            if errors:
                console.print(f"\n[red]✗[/red] {len(errors)} errors occurred")
                for err in errors[:5]:
                    console.print(f"  [dim]{err['name']}:[/dim] {err['error']}")

            if not enriched and not errors:
                console.print("[yellow]No ORCID matches found[/yellow]")
