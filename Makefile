...
clean:
	@if exist imas_codex\__pycache__ rmdir /s /q imas_codex\__pycache__
...
test:
	uv run pytest --cov=imas_codex --cov-report=html --cov-report=term
...
format:
	uv run black imas_codex tests scripts

# Lint with ruff
lint:
	uv run ruff check imas_codex tests scripts

# Fix linting issues
lint-fix:
	uv run ruff check --fix imas_codex tests scripts
