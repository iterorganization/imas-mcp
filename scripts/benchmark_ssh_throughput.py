...
def bench_raw_ssh_pipe(target: str, size_mb: int, iterations: int = 3) -> dict[str, Any]:
    ...
    for _i in range(iterations):
        ...
        results.append({"elapsed_s": round(elapsed, 3), "throughput_mbs": round(throughput_mbs, 1)})
...
"""
    ...
    for _i in range(iterations):
        ...
        if proc.returncode != 0:
            return {"error": f"Remote python failed: {proc.stderr.decode().strip()[:200]}"}
...
def bench_signal_extraction(target: str, n_signals: int, points_per_signal: int,
...
def bench_parallel_ssh(target: str, size_mb: int, n_streams: int,
                       iterations: int = 3) -> dict[str, Any]:
        ...
        def run_stream(stream_id: int) -> dict:
            ...
            proc = subprocess.run(
                ["ssh", "-o", "BatchMode=yes", "-o", "ControlMaster=no",
                 "-o", "ControlPath=none", target, runner_cmd],
                capture_output=True, timeout=300,
            )
            elapsed = time.perf_counter() - t0
            received_mb = len(proc.stdout) / (1024 * 1024)
...
def bench_network_info(target: str) -> dict[str, Any]:
    ...
    try:
        ...
        for line in result.stderr.split("\n"):
            if "cipher:" in line.lower() or "kex:" in line.lower() or "mac:" in line.lower():
...
def main():
    ...
    parser.add_argument("--target", required=True,
                        help="SSH host alias to test (e.g., 'tcv', 'iter')")
    ...
    for size_mb in sizes:
        ...
        if "error" in result:
            ...
            print(f" {result['median_throughput_mbs']} MB/s "
    ...
    if not args.skip_extraction:
        ...
        for label, n_sig, n_pts in workloads:
            ...
            print(f"  {label} ({n_sig}×{n_pts}, ~{raw_mb:.0f} MB) ...", end="", flush=True)
            result = bench_signal_extraction(
                args.target, n_sig, n_pts, iterations=args.iterations,
            )
    ...
    for n in parallel_counts:
        ...
        result = bench_parallel_ssh(
            args.target, parallel_size, n, iterations=args.iterations,
        )
        par_results[f"{n}_streams"] = result
        if "error" in result:
            print(f" ERROR: {result['error']}")
        else:
            print(f" {result['median_aggregate_mbs']} MB/s aggregate "
    ...
    # Best raw throughput
    best_raw = max(
        (v.get("median_throughput_mbs", 0) for v in raw_results.values()
    ...
    # Best python pipe
    best_py = max(
        (v.get("median_throughput_mbs", 0) for v in py_results.values()
         if isinstance(v, dict) and "median_throughput_mbs" in v),
        default=0,
    )
    ...
    # Best parallel
    best_par = max(
        (v.get("median_aggregate_mbs", 0) for v in par_results.values()
    ...
    if best_raw > 0:
        ...
        print(f"  Time to fill GPU mem @ SSH: {time_to_fill:.0f}s ({time_to_fill/60:.1f} min)")
