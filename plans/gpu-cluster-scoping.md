...
The Science Division (SCD) has two corporate objectives for 2026, approved at the Science and Integration Department (SID) level: first, to demonstrate the creation of an AI agent that can autonomously run an IMAS physics code given natural language requests; and second, to develop AI capabilities to understand, label, and map experimental data for scientific analysis, including data from ITER Members and the Magnet Cold Test Facility. The successful delivery of both objectives depends on GPU compute infrastructure that ITER does not currently possess. The existing SDCC GPU estate, comprising shared T4 login GPUs and obsolete P100 accelerators without tensor cores, cannot run the models required for either agentic simulation or large-scale data mapping. This document presents the Science Division's requirements for an on-site GPU facility and the use cases that motivate its acquisition.

Three use cases are presented in order of operational need. The first is a multilingual embedding service for semantic knowledge retrieval and IMAS data mapping, which directly supports the data mapping corporate objective by enabling automated discovery of correspondences between facility-native data formats and the IMAS data model. The second is a local deployment of open-weight agentic LLMs, which supports the agentic simulation objective by removing the third-party rate limits that currently constrain our development velocity to two or three concurrent agents. The third is the development of a Fusion World Model capable of generative simulation of plasmas, a longer-term ambition that builds on the data and infrastructure established by the first two use cases.

Both corporate objectives sit within SID, alongside the IT Division that would procure and host this infrastructure, providing a natural alignment of scientific ambition and technical support. The timing of this investment is important. The infrastructure, competences, and AI pipelines developed now will be mature and tested by the time ITER produces its first plasma. A local GPU facility would also provide an additional layer of data protection by keeping sensitive information on-site, complementing our continued use of corporate AI services such as GitHub Copilot for non-sensitive work.
