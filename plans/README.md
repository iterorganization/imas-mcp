...
| ~~P1f~~ | ~~features/standard-names/40-catalog-layout-hierarchy.md~~ | ✅ Completed — ISNC one-file-per-domain migration; graph-traversal ordering; `COMPUTED_FIELDS` = `{arguments, error_variants}`; `CANONICAL_KEY_ORDER` byte-stable round-trip; `export_scope` manifest + `edge_model_version: plan_39_v1` pin + `FileLock`. | — |
| ~~P1g~~ | ~~features/standard-names/41-catalog-graph-consumer.md~~ | ✅ Completed — ISN-side NetworkX `DiGraph` emitting `HAS_ARGUMENT` / `HAS_ERROR` / `HAS_PREDECESSOR` / `HAS_SUCCESSOR` / `REFERENCES`; 4 MCP tools (`get_standard_name_neighbours`, `get_standard_name_ancestors`, `get_standard_name_descendants`, `shortest_standard_name_path`); catalog-site Mermaid hierarchy blocks + resolved links + sibling nav + COCOS emission; `[graph-local]` extra. | — |
| ~~P1h~~ | ~~features/standard-names/42-polling-workers.md~~ | ✅ Completed — replaced asyncio fan-out with budget-managed polling workers; `BudgetManager` with `pool_admit`; async `_orphan_sweep_tick` for stuck claims. | — |
| ~~P1i~~ | ~~features/standard-names/43-pipeline-rd-fix.md~~ | ✅ Completed — compose-prompt reduction 39K→≤8K tokens; budget split; reviewer-pilot profile; error-gate fixes. | — |
| ~~P1j~~ | ~~features/standard-names/44-sn-graph-renames-batched-embed.md~~ | ✅ Completed — `StandardNameReview` / `HAS_REVIEW` rename; multi-valued `HAS_PHYSICS_DOMAIN`; batch embedding in compose pipeline; `sn clear` command. | — |
| ~~P8.1~~ | ~~features/standard-names/38-grammar-vnext.md~~ (→ `completed/`) | ✅ Completed — six-pool generate/review/refine architecture (Option B); `REFINED_FROM` chained SN nodes; `DocsRevision` via `DOCS_REVISION_OF`; `--min-score` / `--rotation-cap` / `--escalation-model` CLI flags; orphan sweep; 27 tasks shipped in commit `51fb80dd`. | — |
