...
| **P1e** | [features/standard-names/39-graph-relationship-completeness.md](features/standard-names/39-graph-relationship-completeness.md) | Structural edges (COMPONENT_OF, REAL/IMAGINARY_PART_OF, UNCERTAINTY_OF, DEPRECATES/SUPERSEDED_BY) + Tier-2 IN_CLUSTER/AT_PHYSICS_DOMAIN backfill + `sn graph reconcile-structural` CLI. Pre-export reconcile gate. Two rounds of rubber-duck review (Sonnet 4.6, Opus 4.6, GPT-5.4) — corum reached. | New — ready to implement |
| **P1f** | [features/standard-names/40-catalog-layout-hierarchy.md](features/standard-names/40-catalog-layout-hierarchy.md) | ISNC one-file-per-domain migration; explicit graph-traversal ordering; `COMPUTED_FIELDS` contract; `CANONICAL_KEY_ORDER` byte-stable round-trip; `export_scope` manifest + publish-safety hardening. Depends on P1e Tier 1. Corum reached. | New — ready after P1e Tier 1 |
| **P1g** | [features/standard-names/41-catalog-graph-consumer.md](features/standard-names/41-catalog-graph-consumer.md) | ISN-side NetworkX local graph + MCP tools (get_neighbours, get_ancestors, get_descendants, shortest_path); catalog-site Mermaid/link rendering. Depends on P1f. | New — ready after P1f |
