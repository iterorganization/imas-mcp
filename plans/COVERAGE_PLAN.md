...
```
Improve test coverage for [Task X] modules in the imas-codex project.
...
4. Run `uv run pytest --cov=imas_codex -q` to verify coverage
...
### Running Coverage
```bash
uv run pytest --cov=imas_codex --cov-report=term-missing --cov-report=html -q
```
