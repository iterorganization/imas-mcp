...
| `imas_codex/standard_names/workers.py` | Six async worker functions |
...
|---------|---------|-------------|
| `sn generate` | Generate standard names from DD paths or facility signals via LLM pipeline | `--source {dd,signals}`, `--ids`, `--domain`, `--facility`, `--cost-limit`, `--dry-run`, `--force`, `--skip-review`, `--reset-to` |
...
- **drafted**: Generated by `sn generate` (LLM pipeline)
