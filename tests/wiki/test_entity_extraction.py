...
# =============================================================================
# Backward Compatibility: Re-exports from scraper.py
# =============================================================================


class TestScraperReexports:
    """Verify that scraper.py still re-exports all functions."""

    def test_extract_mdsplus_paths(self):
        from imas_codex.discovery.wiki.scraper import extract_mdsplus_paths as fn

        assert callable(fn)

    def test_extract_imas_paths(self):
        from imas_codex.discovery.wiki.scraper import extract_imas_paths as fn

        assert callable(fn)

    def test_extract_units(self):
        from imas_codex.discovery.wiki.scraper import extract_units as fn

        assert callable(fn)

    def test_extract_conventions(self):
        from imas_codex.discovery.wiki.scraper import extract_conventions as fn

        assert callable(fn)

    def test_extract_ppf_paths(self):
        from imas_codex.discovery.wiki.scraper import extract_ppf_paths as fn

        assert callable(fn)

    def test_extract_facility_tool_mentions(self):
        from imas_codex.discovery.wiki.scraper import (
            extract_facility_tool_mentions as fn,
        )

        assert callable(fn)

    def test_facility_entity_extractor(self):
        from imas_codex.discovery.wiki.scraper import FacilityEntityExtractor as cls

        assert cls is FacilityEntityExtractor
    ...
    def test_imas_path_pattern_compat(self):
        """IMAS_PATH_PATTERN should be available via scraper (backward compat)."""
        from imas_codex.discovery.wiki.scraper import IMAS_PATH_PATTERN

        assert IMAS_PATH_PATTERN is not None
        # Should match more IDS than old 12-name pattern
        matches = IMAS_PATH_PATTERN.findall("thomson_scattering/channel/te")
        assert len(matches) >= 1
