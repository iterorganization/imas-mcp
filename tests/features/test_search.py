...
class TestSearchFeatures:
    ...
    async def test_search_with_different_query_types(self, tools):
        ...
        for query in test_queries:
            result = await tools.search_tool.search_dd_paths(query=query, max_results=5)
    ...
    async def test_search_result_limits(self, tools):
        ...
        for limit in limits:
            result = await tools.search_tool.search_dd_paths(
                query="plasma", max_results=limit
            )
    ...
    async def test_empty_query_handling(self, tools):
        ...
        result = await tools.search_tool.search_dd_paths(query="", max_results=5)
...
class TestSearchPathsResultStructure:
    ...
    async def test_search_result_consistency(self, tools):
        ...
        result = await tools.search_tool.search_dd_paths(
            query="temperature", max_results=5
        )
...
class TestSearchErrorHandling:
    ...
    async def test_search_invalid_parameters(self, tools):
        ...
        result = await tools.search_tool.search_dd_paths(query="test", max_results=-1)
    ...
    async def test_search_special_characters(self, tools):
        ...
        for query in special_queries:
            result = await tools.search_tool.search_dd_paths(query=query, max_results=3)
