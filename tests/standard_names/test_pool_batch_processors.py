...
# ---------------------------------------------------------------------------
# Test 6: regen processes batch (happy path)
# ---------------------------------------------------------------------------


class TestRegenProcessesBatch:
    @pytest.mark.asyncio
    async def test_regen_processes_batch(self) -> None:
        """Happy path: regen_increment should be set on candidates."""
        from imas_codex.standard_names.workers import process_regen_batch

        items = _make_batch_items(2)
        mgr = _mock_budget_manager()
        stop = asyncio.Event()

        persisted_candidates: list[list[dict]] = []

        def _capture_persist(candidates, **kwargs):
            persisted_candidates.append(candidates)
        ...
        with (
            ...
            count = await process_regen_batch(items, mgr, stop)

        assert count == 2
        # Verify regen_increment was set
        assert persisted_candidates, "persist should have been called"
        for cand in persisted_candidates[0]:
            assert cand.get("regen_increment") is True
