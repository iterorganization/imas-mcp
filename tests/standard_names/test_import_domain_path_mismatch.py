...
class TestDomainPathMismatch:
    ...
    def test_no_standard_names_dir_structure_errors(self, tmp_path: Path) -> None:
        """File not in standard_names/<domain>.yml layout → error."""
        root = tmp_path / "isnc"
        flat_dir = root / "standard_names"
        flat_dir.mkdir(parents=True)
        # File directly in standard_names/ in a subdirectory (old per-file layout)
        sub = flat_dir / "kinetics"
        sub.mkdir()
        (sub / "electron_temperature.yml").write_text(yaml.safe_dump(_ENTRY_KINETICS))
        ...
        assert "dict" in error_text.lower() or "domain" in error_text.lower()

    def test_correct_domain_path_parses(self, tmp_path: Path) -> None:
        """File in standard_names/kinetics.yml → domain = kinetics, no error."""
        root = tmp_path / "isnc"
        sn_dir = root / "standard_names"
        sn_dir.mkdir(parents=True)
        (sn_dir / "kinetics.yml").write_text(yaml.safe_dump([_ENTRY_KINETICS]))
    ...
    def test_deep_nested_domain_path(self, tmp_path: Path) -> None:
        """standard_names/equilibrium/sub/file.yml → cannot derive domain (not direct child)."""
        root = tmp_path / "isnc"
        ...
        # Deep nested file cannot derive domain from the new layout
        assert len(report.errors) >= 1
