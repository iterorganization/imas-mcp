...
class TestServerSearchCaching:
    ...
    async def test_cache_integration_basic(self, server):
        ...
        server.tools.search_cache.clear()
        ...
        with patch.object(
            server.tools.search_composer, "search", return_value=mock_results
        ):
            # First call should miss cache and execute search
            result1 = await server.tools.search_imas("plasma temperature")

            assert (
                result1.get("cache_hit") is not True
            )  # Should not have cache_hit flag on first call
            assert server.tools.search_cache.stats["misses"] == 1
            assert server.tools.search_cache.stats["sets"] == 1

            # Second identical call should hit cache
            result2 = await server.tools.search_imas("plasma temperature")

            assert result2.get("cache_hit") is True
            assert server.tools.search_cache.stats["hits"] == 1
            assert server.tools.search_cache.stats["sets"] == 1  # Should not increment
    ...
    async def test_cache_with_different_parameters(self, server):
        ...
        with patch.object(
            ...
            await server.tools.search_imas("plasma temperature")
            await server.tools.search_imas("magnetic field")  # Different query

            assert server.tools.search_cache.stats["misses"] == 2
            assert server.tools.search_cache.stats["sets"] == 2
            ...
            assert server.tools.search_cache.stats["misses"] == 5
            assert server.tools.search_cache.stats["sets"] == 5
    ...
    async def test_cache_does_not_store_errors(self, server):
        ...
        with patch.object(
            server.tools.search_composer,
            "search",
            side_effect=Exception("Search failed"),
        ):
            result = await server.tools.search_imas("failing query")

            assert "error" in result
            assert (
                server.tools.search_cache.stats["sets"] == 0
    ...
    async def test_cache_does_not_store_empty_results(self, server):
        ...
        # Mock empty results
        with patch.object(server.tools.search_composer, "search", return_value=[]):
            result = await server.tools.search_imas("empty query")
    ...
    async def test_cache_with_ai_context(self, server):
        ...
        with patch.object(
            ...
            result1 = await server.tools.search_imas(
                "plasma temperature", ctx=mock_context
            )
            ...
            result2 = await server.tools.search_imas(
                "plasma temperature", ctx=mock_context
            )
    ...
    async def test_cache_stats_integration(self, server):
        ...
        with patch.object(
            ...
            await server.tools.search_imas("query1")  # miss + set
            await server.tools.search_imas("query1")  # hit
            await server.tools.search_imas("query2")  # miss + set
            await server.tools.search_imas("query1")  # hit

            stats = server.tools.search_cache.get_stats()
            assert stats["hits"] == 2
    ...
    async def test_cache_clear_functionality(self, server):
        ...
        with patch.object(
            ...
            await server.tools.search_imas("query1")
            await server.tools.search_imas("query2")

            assert server.tools.search_cache.get_stats()["cache_size"] == 2

            # Clear cache
            server.tools.search_cache.clear()

            assert server.tools.search_cache.get_stats()["cache_size"] == 0
